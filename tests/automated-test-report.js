...
`);
    ...
    logResult(category, testName, status, details = "", metrics = {}) {
      ...
      console.log(
        `${emoji} [${category.toUpperCase()}] ${testName}: ${details}`
      );
    ...
    async testFunctionalFeatures() {
      ...
      this.logResult(
        ...
        favoritesWorking
          ? "Favorites elements present"
          : "Favorites elements missing"
      );
      ...
      this.logResult(
        ...
        progressWorking
          ? "Progress elements present"
          : "Progress elements missing"
      );
    }
    ...
    async testDesignElements() {
      ...
      const hasAnimationCSS =
        document.querySelector('link[href*="animations"]') !== null;
      this.logResult(
        "design",
        "Animation Styles",
      ...
      if (questionCard) {
        ...
        const hasGlass =
          cardStyle.backdropFilter && cardStyle.backdropFilter !== "none";
        this.logResult(
          "design",
        ...
        this.logResult(
          ...
          "Glassmorphism Effects",
          "SKIP",
          "Question section not visible"
        );
      }

      // Test 7: Color Contrast
      const textColor =
        rootStyle.getPropertyValue("--text-primary") || "#000000";
    ...
    async testPerformance() {
      ...
      if (performance.memory) {
        ...
        this.logResult(
          "performance",
          "Memory Usage",
          memoryMB < 50 ? "PASS" : memoryMB < 150 ? "WARN" : "FAIL",
          `${memoryMB}MB heap used`,
      ...
      this.logResult(
        ...
        querySelectorTime < 50
          ? "PASS"
          : querySelectorTime < 100
          ? "WARN"
          : "FAIL",
    ...
    printFinalReport() {
      ...
      Object.keys(this.testResults).forEach((category) => {
        ...
        const categoryScore =
          results.length > 0
            ? ((categoryPass / results.length) * 100).toFixed(1)
            : "0.0";

        console.log(
          `📂 ${category.toUpperCase()}: ${categoryScore}% (${categoryPass}/${
            results.length
          } passed)`
        );
      });
      ...
      console.log(
        `   ⚠️ Warnings: ${warnedTests} (${(
          (warnedTests / totalTests) *
          100
        ).toFixed(1)}%)`
      );
      console.log(
      ...
      if (warnedTests > 0) {
        console.log(
          "   🟡 Some areas could be improved for better user experience"
        );
      }
      ...
      console.log(
        "   Use JSON.stringify(window.lastTestReport, null, 2) to export"
      );
    }
  }
